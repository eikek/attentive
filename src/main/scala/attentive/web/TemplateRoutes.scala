...
import attentive.{BuildInfo, Util}
...
object TemplateRoutes {
  ...
  case class Flags(appName: String, base: String, apiBase: String, registrationMode: String)

  object Flags {
    def apply(cfg: Config): Flags =
      Flags(cfg.appName, cfg.baseUrl.asString, (cfg.baseUrl/"api"/"v1").asString, cfg.registration.mode.toString)
    ...
    def apply(cfg: Config): IndexData =
      IndexData(Flags(cfg)
        , Seq(
          ...
          s"/app/assets/${BuildInfo.name}/${BuildInfo.version}/attentive.css"
        )
        , Seq(
          ...
          s"/app/assets/${BuildInfo.name}/${BuildInfo.version}/attentive-app.js"
        )
        ,
        s"/app/assets/${BuildInfo.name}/${BuildInfo.version}/attentive.js"
